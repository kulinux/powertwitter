...
/**
  ...
  */
class TwitterController @Inject()(cc: PostControllerComponents)(implicit ec: ExecutionContext)
  ...
  private def processJsonPost[A]()(implicit request: TwitterRequest[A]): Future[Result] = {
    ...
    def success(input: PostFormInput) = {
      postResourceHandler.create(input).map { post =>
        Created(Json.toJson(post)).withHeaders(LOCATION -> post.id)
      }
    }
